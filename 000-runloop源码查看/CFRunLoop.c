...
typedef struct _per_run_data {
    ...
    uint32_t b;
    uint32_t stopped;
    // 是否忽略唤醒操作，0x57414B45 = 忽略，0x0 = 不忽略
...
static Boolean __CFRunLoopDoBlocks(CFRunLoopRef rl, CFRunLoopModeRef rlm) { // Call with rl and rlm locked
    ...
    while (item) {
        ...
        if (CFStringGetTypeID() == CFGetTypeID(curr->_mode)) {
            ...
            // 当前mode鱼指定mode相等或当前mode为commonMode（字符串）
            // 且commonMode（数组）这个集合中包含制动mode
...
static void __CFRunLoopDoObservers(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFRunLoopActivity activity) {    /* DOES CALLOUT */
    ...
    // rlm中所有可以执行的observer的数组
    CFRunLoopObserverRef *collectedObservers = (cnt <= 1024) ? buffer : (CFRunLoopObserverRef *)malloc(cnt * sizeof(CFRunLoopObserverRef));
    CFIndex obs_cnt = 0;
    // 将可执行的observer放到collectedObservers中
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
    ...
    //如果当前runloop或者runloopModel未停止状态，则直接返回
    ...
    do {
        ...
        // 检测端口，如果端口有时间则跳转到handle_msg （首次执行不会进入判断，因为didDispatchPortLastTime为True）
        
         // 第七步，进入循环开始不断的读取端口信息，如果端口有唤醒信息则唤醒当前runLoop
        if (MACH_PORT_NULL != dispatchPort && !didDispatchPortLastTime) {
#if DEPLOYMENT_TARGET_MACOSX || DEPLOYMENT_TARGET_EMBEDDED || DEPLOYMENT_TARGET_EMBEDDED_MINI
            msg = (mach_msg_header_t *)msg_buffer;
            if (__CFRunLoopServicedoMachPort(dispatchPort, &msg, sizeof(msg_buffer), &livePort, 0, &voucherState, NULL)) {
                goto handle_msg;
            }
#elif DEPLOYMENT_TARGET_WINDOWS
